#!/usr/bin/env python3
"""
UR Joint + Gripper Trajectory Visualizer

Subscribes to:
    /servo_angles   — U-Arm master arm angles (7 floats, deg)
    /robot_state    — UR actual state (6 joint deg + gripper 0-1)
...
Blue = U-Arm master,  Red = UR actual
...
# ── Config ────────────────────────────────────────────────────────────────────
JOINT_MAP = [0, 1, 2, 3, 5, 4]  # must match servo2ur.py
...
class TrajVizNode(Node):
    def __init__(self):
        ...
        self._ur_buf = [deque(maxlen=MAX_SAMPLES) for _ in range(7)]
    ...
    def _sample(self):
        with self._lock:
            ...
            self._ur_buf[6].append(self._state[6] * 100.0)
...
def run_plot(node: TrajVizNode):
    ...
    title_text = fig.text(
        0.5,
        0.965,
        "Joint & Gripper Trajectories   |   Blue: U-Arm    Red: UR",
        ha="center",
        fontsize=12,
        color="#cccccc",
        fontweight="bold",
