#!/usr/bin/env python3
"""
test_ur.py - UR connectivity and basic motion test (no ROS required).
...
Usage:
    python3 test_ur.py [ip]
    python3 test_ur.py 10.0.0.1       # default - match servo2ur.py UR_IP
...
# Must match UR_IP in servo2ur.py AND ur_pub.py
UR_IP = "10.0.0.1"  # ← [ADJUST] change to your UR controller IP

# Safe home position - keep in sync with servo2ur.py UR_HOME_DEG
# ← [ADJUST] verify this pose is collision-free before moving a new model
HOME_DEG = [0.0, -90.0, 90.0, -90.0, -90.0, 0.0]
JOINT_NAMES = ["shoulder_pan", "shoulder_lift", "elbow", "wrist_1", "wrist_2", "wrist_3"]


def main():
    ip = sys.argv[1] if len(sys.argv) > 1 else UR_IP
    print(f"UR connectivity test  -  IP: {ip}")
    print("=" * 48)
    ...
    try:
        ...
        print("       Check: UR powered on, Ethernet cable, correct IP.")
        sys.exit(1)
