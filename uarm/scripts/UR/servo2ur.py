#!/usr/bin/env python3
"""
UR + Weiss Robotics CRG 30-050 Teleoperation Node
...
Hardware:
    - UR connected via Ethernet (RTDE, port 30004)
...
# UR home/initial joint position (degrees).
# [shoulder_pan, shoulder_lift, elbow, wrist1, wrist2, wrist3]
# ← [ADJUST] tune for your workspace; arm length/reach differs across UR models
UR_HOME_DEG = [45.0, -20.0, -140.0, -40.0, -270.0, 0.0]

# Reorder master-arm servo indices to UR joint indices.
# JOINT_MAP[i] = which servo channel drives UR joint i.
# Swap master motor 4 → UR joint 5, master motor 5 → UR joint 4.
JOINT_MAP = [0, 1, 2, 3, 5, 4]

# Per-joint scale factors (sign correction) applied after reordering.
# ← [ADJUST] signs and magnitudes depend on master-arm kinematics, not UR model
...
# UR joint velocity limit (rad/s) — safety cap; set high enough that the UR
# can always keep up with the master so no catch-up backlog builds after a stop.
# ← [ADJUST] hardware max: UR3/UR5 ~2.09 rad/s, UR10/UR16 ~1.57 rad/s;
#             lower this value for heavier payloads or when near joint limits
...
class URTeleopNode(Node):
    """
    Reads Uarm master-arm joint angles, maps them to the UR frame,
    streams joint commands via RTDE servoJ, and drives the CRG 30-050 gripper.
    """

    def __init__(self):
        super().__init__("ur_teleop_node")

        # ── UR RTDE ──────────────────────────────────────────────
        self.get_logger().info(f"[URTeleop] (1/4) Connecting RTDE control to {UR_IP}...")
        self.rtde_c = rtde_control.RTDEControlInterface(UR_IP)
        self.get_logger().info(f"[URTeleop] (2/4) Connecting RTDE receive to {UR_IP}...")
        self.rtde_r = rtde_receive.RTDEReceiveInterface(UR_IP)
        self.get_logger().info("[URTeleop] RTDE connected.")

        # ── Weiss CRG 30-050 ──────────────────────────────────────
        self.get_logger().info(f"[URTeleop] (3/4) Initialising gripper on {GRIPPER_PORT} (takes ~5 s)...")
        self.gripper = WeissCRGGripper(GRIPPER_PORT, GRIPPER_BAUDRATE, logger=self.get_logger())
        self.get_logger().info("[URTeleop] Gripper init done. Running home cycle (takes ~10 s)...")
        if not self.gripper.home():
            self.get_logger().warning("[URTeleop] Gripper homing failed — check LED and cable.")
        else:
            self.get_logger().info("[URTeleop] Gripper homed.")

        # ── UR home position ──────────────────────────────────────
        self.home_rad = np.radians(UR_HOME_DEG)
        self.get_logger().info(f"[URTeleop] (4/4) Moving UR to home: {UR_HOME_DEG} deg (slow moveJ)...")
        self.rtde_c.moveJ(self.home_rad.tolist(), speed=0.3, acceleration=0.3)
        self.get_logger().info("[URTeleop] UR at home position.")
        ...
        self.get_logger().info(
            f"[URTeleop] *** READY *** — streaming servoJ at {CONTROL_HZ} Hz, "
    ...
    def _servo_callback(self, msg: Float64MultiArray):
        ...
        if data.size < 7:
            if not self._servo_warned:
                self.get_logger().warning("[URTeleop] /servo_angles needs 7 values (6 joints + gripper)")
    ...
    def _gripper_loop(self):
        ...
        while rclpy.ok():
            ...
            if want_open != last_open:
                ...
                last_open = want_open
                self.get_logger().info(f"[URTeleop] Gripper {'opening' if want_open else 'closing'}")
            time.sleep(dt)
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            try:
                ...
                self.get_logger().warning(f"[URTeleop] servoJ error: {e}")
            ...
            self.get_logger().debug(f"[URTeleop] action: {[f'{v:.2f}' for v in action_data]}")
        ...
        self.get_logger().info("[URTeleop] servoJ stopped.")
        self.gripper.close()
