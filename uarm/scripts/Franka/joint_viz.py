#!/usr/bin/env python3
"""
Franka Panda Joint + Gripper Trajectory Visualizer

Subscribes to:
    /servo_angles   — U-Arm master arm angles (7 floats, deg)
    /robot_state    — Panda actual state (7 joint deg + gripper 0-1)
    /is_recording   — Bool from episode_recorder

Layout: 4 rows x 2 cols (8 slots, all used)
    J0  J1
    J2  J3
    J4  J5
    J6  Gripper

Blue = U-Arm master,  Red = Panda actual
...
# ── Config ────────────────────────────────────────────────────────────────────
# JOINT_MAP[panda_i] = master servo index driving Panda joint i; must match servo2franka.py
JOINT_MAP = [0, 1, 2, 3, 4, 5, -1]

# 8 channels: joints 0-6, then gripper
CHANNEL_NAMES  = ["J0", "J1", "J2", "J3", "J4", "J5", "J6", "Gripper"]
CHANNEL_YLABELS = ["deg"] * 7 + ["%"]
...
class TrajVizNode(Node):
    def __init__(self):
        ...
        self._uarm_buf = [deque(maxlen=MAX_SAMPLES) for _ in range(8)]
        self._ur_buf   = [deque(maxlen=MAX_SAMPLES) for _ in range(8)]
    ...
    def _cb_servo(self, msg):
        if len(msg.data) >= 7:
            with self._lock:
                self._servo = list(msg.data[:7]) + [self._servo[7]]

    def _cb_state(self, msg):
        if len(msg.data) >= 8:
            with self._lock:
                self._state = list(msg.data[:8])
    ...
    def _sample(self):
        with self._lock:
            ...
            for i in range(7):
                master_i = JOINT_MAP[i]
                self._uarm_buf[i].append(self._servo[master_i] if master_i >= 0 else 0.0)
                self._ur_buf[i].append(self._state[i])
            # gripper: servo[6] deg, state[7] normalised → scale to %
            self._uarm_buf[7].append(self._servo[6])
            self._ur_buf[7].append(self._state[7] * 100.0)
...
def run_plot(node: TrajVizNode):
    ...
    gs  = gridspec.GridSpec(
        4, 2, figure=fig, hspace=0.55, wspace=0.30,
        left=0.07, right=0.97, top=0.93, bottom=0.06,
    )

    # 8 subplots: all 4 rows, both columns
    ...
    lines_uarm, lines_ur, vlines, val_texts = [], [], [], []
    ...
    for i, ax in enumerate(axes):
        ...
        label_u = "U-Arm" if i < 7 else "Master (deg)"
        label_r = "Panda" if i < 7 else "Panda (%)"
        ...
        ax.legend(fontsize=8, loc="upper left", ncol=2, handlelength=1.4,
                  columnspacing=0.8, borderpad=0.5)

        vt = ax.text(
            0.99, 0.97, "",
            transform=ax.transAxes, fontsize=8, color="#ffcc44",
    ...
    title_text = fig.text(
        0.5, 0.965,
        "Joint & Gripper Trajectories   |   Blue: U-Arm    Red: Panda",
        ha="center", fontsize=12, color="#cccccc", fontweight="bold",
    )
    ...
    while rclpy.ok() and plt.fignum_exists(fig.number):
        t_arr, uarm_data, ur_data, rec, cur_servo, cur_state = node.get_data()
        n   = len(t_arr)
        pts = 0

        for i in range(8):
            pts = min(n, len(uarm_data[i]), len(ur_data[i]))
            if pts > 1:
                t_p = t_arr[:pts]
                lines_uarm[i].set_data(t_p, uarm_data[i][:pts])
                lines_ur[i].set_data(t_p, ur_data[i][:pts])
            else:
            ...
            if i < 7:
                master_i = JOINT_MAP[i]
                u_val = cur_servo[master_i] if master_i >= 0 else 0.0
                val_texts[i].set_text(f"U:{u_val:+.1f}°  R:{cur_state[i]:+.1f}°")
            else:
                val_texts[i].set_text(f"U:{cur_servo[6]:+.1f}°  R:{cur_state[7] * 100:.0f}%")
