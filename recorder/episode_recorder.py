...
Topics consumed:
    ...
    /robot_eef_pose Float64MultiArray (6,)   → TCP pose [x, y, z, rx, ry, rz] metres/rad
...
HDF5 layout written per episode (aloha-compatible, pi0.5/ACT/Diffusion-Policy ready):
    ...
    /observations/eef_pose                       (T, 6)  float64  [x, y, z, rx, ry, rz] metres/rad
    /action                                      (T, 7)  float64
...
class DataBuffer:
    ...
    def __init__(self, node: Node):
        ...
        self._eef_pose: np.ndarray | None = None
        ...
        node.create_subscription(Float64MultiArray, "/robot_eef_pose", self._cb_eef_pose, 10)

        # ── Wait for the required topics (cam1, cam2, state, vel, action, eef_pose) ────
        node.get_logger().info(
            "[Recorder] Waiting for required topics (cam_1, cam_2, robot_state, robot_vel, robot_action, robot_eef_pose) …"
        )
    ...
    def _cb_eef_pose(self, msg: Float64MultiArray):
        with self._lock:
            self._eef_pose = np.array(msg.data, dtype=np.float64)

    # ── public API ────────────────────────────────────────────────────────────

    def _core_ready(self) -> bool:
        with self._lock:
            return all(x is not None for x in [self._cam1_bgr, self._cam2_bgr, self._state, self._vel, self._action, self._eef_pose])

    def is_ready(self) -> bool:
        with self._lock:
            core = all(x is not None for x in [self._cam1_bgr, self._cam2_bgr, self._state, self._vel, self._action, self._eef_pose])
    ...
    def get_snapshot(self) -> dict | None:
        with self._lock:
            ...
            required = [self._cam1_bgr, self._cam2_bgr, self._state, self._vel, self._action, self._eef_pose]
            if self.has_front_camera:
...
def save_episode_hdf5(episode: dict, path: str, prompt: str, task: str, hz: float) -> None:
    ...
    eef_pose = np.round(np.stack(episode["eef_pose"]), 6)
    ...
    with h5py.File(path, "w", rdcc_nbytes=1024**2 * 32) as root:
        ...
        obs.create_dataset("eef_pose", data=eef_pose, dtype="float64", compression="gzip", compression_opts=4)
...
def run(args) -> None:
    ...
    def reset_buf():
        ...
        episode_buf = {"cam1_rgb": [], "cam2_rgb": [], "state": [], "vel": [], "action": [], "eef_pose": []}
            ...
            if rec_state == RECORDING:
                ...
                if now - last_step_t >= dt:
                    ...
                    if snap is not None:
                        ...
                        episode_buf["eef_pose"].append(snap["eef_pose"])
                        step_ts.append(now)
